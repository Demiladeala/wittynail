  ...
  return (
    <div className="font-jost">
      ...
      <div className="max-w-[90%] w-[90%] overflow-x-hidden mx-auto">
        ...
        <div
          className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 items-start w-full mx-auto max-w-full
                gap-x-4 md:gap-x-6 gap-y-3 md:gap-y-5 justify-start flex-wrap"
        >
          {data === null ? (
            [...Array(6)].map((_, index) => <ProductLoader key={index} />)
          ) : data && (data.length === 0 || data === null) ? (
            <div className="w-full my-40 text-center col-span-full">
              No products found
            </div>
          ) : (
            data &&
